/*
 * Modified by Funtum in 2026 for the AuroraChat Wii U Plugin.
 * Changes: Removed the OpenGL Mathematics (GLM) library dependency and halved the blur scale to make the distance of characters further.
 *
 * Originally sourced from: https://github.com/wiiu-env/NotificationModule/raw/refs/heads/main/src/gui/SchriftGX2.cpp
 */
...
uint16_t SchriftGX2::drawText(int16_t x, int16_t y, int16_t z, const wchar_t *text, int16_t pixelSize, const float *color, uint16_t textStyle, uint16_t textWidth, const float &textBlur, const float &colorBlurIntensity, const float *blurColor) {
    (void) textWidth;
    if (!text) {
        return 0;
    }
...
*/
void SchriftGX2::copyTextureToFramebuffer(GX2Texture *texture, int16_t x, int16_t y, int16_t z, const float *color, const float &defaultBlur, const float &blurIntensity, const float *blurColor) {
    static const float imageAngle = 0.0f;
    static const float blurScale  = 1.0f;
