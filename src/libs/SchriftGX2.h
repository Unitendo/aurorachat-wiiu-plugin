 ...
 * Changes: Removed the OpenGL Mathematics (GLM) library dependency.
 *
 * Originally sourced from: https://github.com/wiiu-env/NotificationModule/raw/refs/heads/main/src/gui/SchriftGX2.h
 */
...
private:
    ...
    void copyTextureToFramebuffer(GX2Texture *tex, int16_t screenX, int16_t screenY, int16_t screenZ, const float *color, const float &textBlur, const float &colorBlurIntensity, const float *blurColor);
...
public:
    ...
    uint16_t drawText(int16_t x, int16_t y, int16_t z, const wchar_t *text, int16_t pixelSize, const float *color,
                      uint16_t textStyling, uint16_t textWidth, const float &textBlur, const float &colorBlurIntensity, const float *blurColor);
