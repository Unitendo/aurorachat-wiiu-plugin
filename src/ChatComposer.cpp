...
void ChatComposer::PressSelected() {
    if (mRow < kNumLetterRows) {
        ...
        switch (mCol) {
            case 0: mBuffer.push_back(' ');                    break; // Space
            case 1: if (!mBuffer.empty()) mBuffer.pop_back();  break; // Backspace
            case 2: Close(false);                              break; // Cancel
            case 3: Close(true);                               break; // Send
        }
    }
}
